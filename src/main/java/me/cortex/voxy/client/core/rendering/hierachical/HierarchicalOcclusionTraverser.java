...
public class HierarchicalOcclusionTraverser {
    ...
    private void uploadUniform(Viewport<?> viewport, TraversalMode mode) {
...

        {
            ...
            int requestCount = mode == TraversalMode.MAIN
                    ? Math.max(0, Math.min(MAX_REQUEST_QUEUE_SIZE, requestSize))
                    : 0;
            MemoryUtil.memPutInt(ptr, requestCount); ptr += 4;
        }
    ...
    public void doTraversal(Viewport<?> viewport) {
        this.doTraversal(viewport, TraversalMode.MAIN);
    }

    public void doTraversal(Viewport<?> viewport, TraversalMode mode) {
