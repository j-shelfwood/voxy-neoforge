...
public class MDICSectionRenderer extends AbstractSectionRenderer<MDICViewport, BasicSectionGeometryData> {
    ...
    private final Shader shadowCommandGenShader = Shader.make()
    ...
    private void uploadUniformBuffer(MDICViewport viewport) {
        ...
        MemoryUtil.memPutFloat(ptr, viewport.shadowMapBias); ptr += 4;
    ...
    public void renderShadow(MDICViewport viewport) {
        ...
        // Iris owns the shadow framebuffer and its projection. Only bind the
        // Voxy geometry buffers here; the normal gbuffer pipeline is not used.
        this.uploadUniformBuffer(viewport);
        glDisable(GL_CULL_FACE);
        glDisable(GL_BLEND);
        glEnable(GL_DEPTH_TEST);
