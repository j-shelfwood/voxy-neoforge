...
public class IrisVoxyRenderPipeline extends AbstractRenderPipeline {
    ...
    protected int setup(Viewport<?> viewport, int sourceFramebuffer, int srcWidth, int srcHeight) {
        int targetWidth = glGetTextureLevelParameteri(this.data.opaqueDrawTargets[0], 0, GL_TEXTURE_WIDTH);
        int targetHeight = glGetTextureLevelParameteri(this.data.opaqueDrawTargets[0], 0, GL_TEXTURE_HEIGHT);
        ...
        if (viewport.width != targetWidth || viewport.height != targetHeight) {
            Logger.warn("Correcting Voxy viewport from " + viewport.width + "x" + viewport.height
                    + " to Iris render target " + targetWidth + "x" + targetHeight);
