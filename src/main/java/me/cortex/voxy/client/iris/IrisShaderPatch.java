...
public class IrisShaderPatch {
    ...
    public boolean supportsEuphoriaShadowCaster() {
        if (this.patchData.shadowCasterVersion != 1
                || this.patchData.uniforms == null
                || this.patchData.samplers == null) {
            return false;
        }

        var uniforms = List.of(this.patchData.uniforms);
        return uniforms.contains("shadowModelView")
                && uniforms.contains("shadowProjection")
                && this.patchData.samplers.containsKey("shadowtex0");
    }
